...
def get_network():
    ...
    print(f"Network loaded: {len(G.nodes)} nodes, {len(G.edges)} edges")
    G = mark_ridden_edges(G)
    return G

def mark_ridden_edges(G):
    print("Mapping your past rides to the road network to find unridden roads...")
    csv_path = os.path.join(DATA_DIR, "my_rides.csv")
    if not os.path.exists(csv_path):
        print("No my_rides.csv found. Novelty factor will not be applied.")
    ...
    X, Y = [], []
    ride_indices = []
    for ride_idx, p in enumerate(df["polyline"]):
        coords = pl.decode(p)
    ...
    if X:
        ...
        for ride_idx, counts in ride_edge_counts.items():
            for (u, v), c in counts.items():
                ...
                if not edge_data: edge_data = G.get_edge_data(v, u)
                if not edge_data: continue
                
                data = edge_data.get(0, list(edge_data.values())[0])
                length = data.get("length", 0)
                
                # If it is a long edge and only got a stray ping or two, it's a cross-street snapping error.
                if length > 200 and c <= (length / 300.0):
                    continue
                    
                if G.has_edge(u, v):
                    for k in G[u][v]:
                        G[u][v][k]["ridden_count"] += 1
                if G.has_edge(v, u):
                    for k in G[v][u]:
                        G[v][u][k]["ridden_count"] += 1
                    
    # Fill in tiny gaps (like intersections <20m) between ridden nodes to fix GPS snapping skips
    ridden_nodes = set()
    for u, v, k, d in G.edges(keys=True, data=True):
    ...
    ridden_total = sum(1 for _,_,_,d in G.edges(keys=True, data=True) if d.get("ridden_count", 0) > 0)
    print(f"Marked {ridden_total} out of {len(G.edges)} edges as previously ridden.")
...
def calculate_novelty_percentage(G, path):
    novel_dist = 0
    total_dist = 0
    for u, v in zip(path[:-1], path[1:]):
        edge_data = G.get_edge_data(u, v)
        if edge_data is None: edge_data = G.get_edge_data(v, u)
        ...
        if data.get("ridden_count", 0) == 0:
            novel_dist += dist
    return (novel_dist / total_dist * 100) if total_dist > 0 else 0
...
def save_route_features(results, G):
    ...
    for i, r in enumerate(results[:5]):
        rows.append({
            ...
            "route_segments":  encode_path_to_segments(G, r["path"]),
        })
...
def encode_path_to_segments(G, path):
    import polyline as pl
    import json
    segments = []
    current_coords = []
    ...
    for u, v in zip(path[:-1], path[1:]):
        ...
        if "geometry" in data:
            ...
            edge_coords = [(float(G.nodes[u]["y"]), float(G.nodes[u]["x"])), 
...
def show_routes_in_browser(G, results, top_n=5):
    ...
    for i, r in enumerate(results[:top_n]):
        label   = f"#{i+1} | Score: {r['score']}/10 | Novelty: {r['novelty']:.1f}% | {r['dist_miles']}mi"
        segments_json = encode_path_to_segments(G, r["path"])
        segments = json.loads(segments_json)

        for seg in segments:
            latLngs = pl.decode(seg["polyline"])
            color = colors[i % len(colors)] if seg["is_new"] else "#555555"
            weight = 5 if seg["is_new"] else 3
            opacity = 0.9 if seg["is_new"] else 0.4
...
if __name__ == "__main__":
    ...
    while len(results) < NUM_ROUTES and attempts < NUM_ROUTES * 10:
        ...
        results.append({
            ...
            "novelty":    calculate_novelty_percentage(G, path),
        })

    # Sort primarily by novelty, then score
    results.sort(key=lambda x: -(x["novelty"]/20 + x["score"]))

    print(f"\nTop 5 Novel routes out of {len(results)} generated:\n")
    for i, r in enumerate(results[:5]):
        print(f"  #{i+1}  Score: {r['score']}/10  |  Novelty: {r['novelty']:.1f}%  |  {r['dist_miles']}mi  |  {r['elev_ft']}ft")

    print("\nSaving top 5 as GPX files...")
    for i, r in enumerate(results[:5]):
        coords = path_to_coords(G, r["path"])
        name   = f"novel_route_{i+1}_{r['dist_miles']}mi_{r['novelty']:.0f}p_novel"
